...
let __lastCodeAt = 0;
const __DUPLICATE_GUARD_MS = 1500;

async function __fillCameras() {
  try {
    ...
    // Selecciona por defecto la última (suele ser trasera en móviles)
    if (devices.length > 0) {
      select.value = devices[devices.length - 1].id;
    }
  } catch (err) {
    console.error('No se pudieron obtener cámaras:', err);
    alert('No se detectaron cámaras o no hay permisos. Verifica HTTPS y permisos del navegador.');
  }
}

function __onDetected(decodedText, decodedResult) {
  const now = Date.now();
  // Evitar lecturas repetidas muy seguidas del mismo código
  if (decodedText && now - __lastCodeAt > __DUPLICATE_GUARD_MS) {
    __lastCodeAt = now;

    // Beep opcional
    const beep = document.getElementById('beepAudio');
    const beepToggle = document.getElementById('beepToggle');
    if (beep && beepToggle && beepToggle.checked) {
      beep.currentTime = 0;
      beep.play().catch(() => {/* silencio si el navegador bloquea autoplay */});
    }

    // Usa tu lógica ya existente
...
async function startCameraScanner() {
  ...
  // Formatos de barras y QR que queremos soportar
  const formats = [
    ...
    Html5QrcodeSupportedFormats.QR_CODE // por si escaneas QR también
  ];
  ...
  const config = {
    fps: 12,
    // Puedes usar un rectángulo más "panorámico" para barras:
    qrbox: { width: 300, height: 140 },
    aspectRatio: 1.7778, // 16:9
    // Mejor soporte para códigos de barras si el navegador lo permite:
    experimentalFeatures: { useBarCodeDetectorIfSupported: true }
  };

  try {
    const cameraConfig = deviceId ? { deviceId: { exact: deviceId } } : { facingMode: "environment" };
    await __html5qrcode.start(cameraConfig, config, __onDetected, () => {});
    __scanning = true;

    // UI
    const btnStart = document.getElementById('btnStartScan');
    const btnStop  = document.getElementById('btnStopScan');
    if (btnStart && btnStop) { btnStart.disabled = true; btnStop.disabled = false; }
  } catch (err) {
    console.error('No se pudo iniciar el escáner:', err);
    alert('No se pudo iniciar la cámara. Verifica permisos, HTTPS y que otra app no la esté usando.');
  }
}
...
async function stopCameraScanner() {
  ...
  try {
    ...
    console.warn('Error al detener/limpiar el escáner:', err);
  } finally {
    __scanning = false;
    const btnStart = document.getElementById('btnStartScan');
    const btnStop  = document.getElementById('btnStopScan');
    if (btnStart && btnStop) { btnStart.disabled = false; btnStop.disabled = true; }
  }
}

// Exponer por si quieres llamarlo desde otras partes
window.startCameraScanner = startCameraScanner;
window.stopCameraScanner  = stopCameraScanner;

// Enlazar botones y preparar cámaras cuando entras a la pestaña "Escanear"
(function __wireScanPage() {
  // Al cambiar de página
  const _origSwitch = window.switchPage;
  window.switchPage = async function(page) {
    // Primero ejecuta el switch original
    _origSwitch(page);

    if (page === 'scan') {
      await __fillCameras();
      // Enlazar botones (una única vez)
      document.getElementById('btnStartScan')?.addEventListener('click', startCameraScanner, { once: true });
      document.getElementById('btnStopScan') ?.addEventListener('click', stopCameraScanner);
    } else {
      // Si salgo de "Escanear", detengo la cámara
      stopCameraScanner();
    }
  };
})();
