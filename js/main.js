...
const switchPage = (page) => {
  ...
  if (page === 'dashboard') {
    ...
    loadProducts();
    // Si salimos de "scan", apaga la cámara
    stopScanner();
    ...
    // Preparar escaneo: abre modal de permiso y, al aceptar, inicia la cámara
    setupScanPage();
  } else {
    // Cualquier otra, apagar por si acaso
    stopScanner();
  }
};
...
let scanInitDone = false;

const scannerStatusEl = () => document.querySelector('.scanner-status-text');
...
async function startScanner() {
  try {
    ...
    if (!cameras || cameras.length === 0) {
      if (scannerStatusEl()) scannerStatusEl().textContent = 'No se encontraron cámaras';
      return;
    }

    // Elegir la trasera si existe
    const backCam = cameras.find(c => /back|rear|trás|atras|environment/i.test(c.label));
    const cameraId = (backCam || cameras[0]).id;

    // Configuración: soportar CODE_128 + otros comunes
    const config = {
      fps: 10,
      // La librería inyecta su propio recorte, nosotros ya tenemos overlay visual
    ...
    // Callback de éxito: manda el texto leído a tu handleScan existente
    const onScanSuccess = (decodedText/*, decodedResult*/) => {
      // Beep opcional (puede fallar si no hubo interacción del usuario)
      const beep = document.getElementById('beepAudio');
      if (beep) beep.play().catch(() => { /* silencioso */ });

      // Tu función ya muestra el producto; sanitiza dígitos dentro de handleScan
      handleScan(decodedText);
    };

    // Callback de error por frame (ruidoso; mejor no loguear por performance)
    const onScanFailure = () => { /* noop */ };

    await html5Qrcode.start({ deviceId: { exact: cameraId } }, config, onScanSuccess, onScanFailure);
    isScannerRunning = true;
    if (scannerStatusEl()) scannerStatusEl().textContent = 'Escaneando…';
  } catch (err) {
    console.error('Error iniciando escáner', err);
    if (scannerStatusEl()) scannerStatusEl().textContent = 'No se pudo iniciar la cámara';
  }
}
...
async function stopScanner() {
  try {
    if (html5Qrcode && isScannerRunning) {
      if (scannerStatusEl()) scannerStatusEl().textContent = 'Deteniendo cámara…';
      await html5Qrcode.stop();     // detiene stream
      html5Qrcode.clear();          // limpia <div id="reader">
      isScannerRunning = false;
      if (scannerStatusEl()) scannerStatusEl().textContent = 'Cámara detenida';
    }
  } catch (err) {
    console.warn('stopScanner falló', err);
  }
}

// Lógica de la página "scan": abrir modal y enganchar botones
function setupScanPage() {
  const modal = document.getElementById('cameraPermissionModal');
  const btnGrant = document.getElementById('permGrantBtn');
  const btnCancel = document.getElementById('permCancelBtn');
  const btnClose  = document.getElementById('permCloseBtn');

  if (!scanInitDone) {
    // Evitar duplicar listeners si el usuario entra/sale de la página
  ...
  // Abrir modal al entrar a la página de escaneo
  modal?.classList.add('active');
  modal?.setAttribute('aria-hidden', 'false');
}

// Exponer por si necesitas depurar manualmente
window.startScanner = startScanner;
window.stopScanner  = stopScanner;
window.setupScanPage = setupScanPage;