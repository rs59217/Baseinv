...
// ========================
// ESCÁNER DE BARRAS (auto-start en pestaña "Escanear")
// ========================
...
function __onDetected(decodedText /*, decodedResult */){
  ...
  // usar la lógica existente (ya actualiza #scanResult)
  try { handleScan(decodedText); } catch(e){ console.error(e); }
}
...
async function startLiveScan(){
  ...
  if (!__html5qrcode){
    __html5qrcode = new Html5Qrcode(containerId, {
  ...
  const config = {
    fps: 12,
    // caja panorámica (mejor para barras) dentro del video
    qrbox: { width: 300, height: 140 },
    aspectRatio: 1.7778,
    // usa API nativa si está disponible (mejor para barras)
    experimentalFeatures: { useBarCodeDetectorIfSupported: true }
  };

  try{
    // Prioriza cámara trasera
    await __html5qrcode.start({ facingMode: "environment" }, config, __onDetected, () => {});
    __scanning = true;
  }catch(err){
    ...
    if (help){
      help.innerHTML = `
        <div class="scan-product-info" style="border-color: var(--warning-color);">
          <h3 style="color: var(--warning-color);"><i class="fas fa-exclamation-triangle"></i> Cámara no disponible</h3>
          <p>Permite el acceso a la cámara o usa la barra inferior para ingresar el código manualmente.</p>
        </div>
      `;
    }
    // Enfocar barra manual
...
async function stopLiveScan(){
  ...
  }catch(e){ console.warn('Error al detener cámara:', e); }
  finally{ __scanning = false; }
}

// ---- Enlazar al flujo de navegación existente ----
(function(){
  const originalSwitchPage = window.switchPage;
  window.switchPage = function(page){
    originalSwitchPage(page);
    if (page === 'scan'){
      // Abre de una: auto-start
      startLiveScan();
    }else{
      // Si salimos de Escanear, detenemos la cámara
      stopLiveScan();
    }
  };
})();
...
// ========================
// PERMISO DE CÁMARA + ESCÁNER (auto al entrar a "Escanear")
// ========================
let __html5qrcode = null;
let __scanning = false;
let __lastCodeAt = 0;
const __DUP_MS = 1200; // anti-duplicados

function __onDetected(decodedText){
  const now = Date.now();
  if (!decodedText || now - __lastCodeAt < __DUP_MS) return;
  __lastCodeAt = now;
  ...
  try { handleScan(decodedText); } catch(e){ console.error('handleScan error:', e); }
}
 ...
 * - Muestra modal explicando por qué (UX).
 * - Llama getUserMedia para disparar el prompt nativo.
 * - Si concede: cierra stream "temporal" y arranca el escáner html5-qrcode.
 */
async function requestCameraPermissionAndStart(){
  try{
    ...
    // Cerramos inmediatamente este stream temporal (solo era para pedir permiso)
    tmpStream.getTracks().forEach(t => t.stop());
    __closePermissionModal();
    // Arrancamos el lector real
    await __startScanner();
  }catch(err){
    console.warn('Permiso denegado o error al solicitar cámara:', err);
    __closePermissionModal();
    // Muestra ayuda y enfoca input manual
    const help = document.getElementById('scanResult');
    if (help){
      help.innerHTML = `
        <div class="scan-product-info" style="border-color: var(--danger-color);">
          <h3 style="color: var(--danger-color);"><i class="fas fa-times-circle"></i> No se pudo usar la cámara</h3>
...
/**
 * Comportamiento al entrar a la pestaña "Escanear":
 * - Si el navegador soporta permisos y ya está "granted": arranca directo.
 * - Si está "prompt" o no sabemos: mostramos el modal para que el usuario autorice.
 * - Si está "denied": mostramos fallback al input manual.
 */
async function __prepareScanPage(){
  ...
  // Chequeo Permissions API (best effort — no todos los navegadores lo exponen)
  try{
    if (navigator.permissions && navigator.permissions.query){
      const status = await navigator.permissions.query({ name: 'camera' });
      // Algunos navegadores devuelven "granted" | "denied" | "prompt"
      if (status.state === 'granted'){
        await __startScanner(); // ya había permiso → empieza sin modal
        return;
      }
      if (status.state === 'denied'){
        // Permiso denegado previamente → avisa y enfoca input manual
        const help = document.getElementById('scanResult');
        if (help){
          help.innerHTML = `
            <div class="scan-product-info" style="border-color: var(--warning-color);">
              <h3 style="color: var(--warning-color);"><i class="fas fa-exclamation-circle"></i> Acceso a cámara denegado</h3>
      ...
      // state === 'prompt' → mostramos el modal para pedir permiso
      __openPermissionModal();
      return;
    }
    // Sin Permissions API → mostramos modal y que el usuario otorgue con botón
    __openPermissionModal();
  }catch(e){
    console.warn('Permissions API no disponible o error:', e);
    __openPermissionModal();
  }
}

// Enganchar al flujo de navegación existente
(function(){
  const originalSwitchPage = window.switchPage;
  window.switchPage = function(page){
    originalSwitchPage(page);
    if (page === 'scan'){
      __prepareScanPage(); // <-- aquí pedimos permiso y arrancamos
    }else{
      __stopScanner();
    }
  };
